...
const PractiseKeyboard = ({nextKey,clickedKey,searchKey}) => {
 ...
 const getKeyClass = (key) => {
  // Check if the current key is the nextKey
  
  if (nextKey === key) {
    
    return "bg-[#D5E94E] text-black"; // Green background for the nextKey

  }

  // Check if the clickedKey does not match nextKey, apply red background for incorrect keys
  if (clickedKey && clickedKey !== searchKey && clickedKey === key) {
    
    return "bg-[#FF9494] text-black"; // Red background for incorrect key
  }

  // Default style for all other keys
  return "bg-[#191A1F] text-gray-400";
};
...
export default PractiseKeyboard;
